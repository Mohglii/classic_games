...
import os
# ----------------------------------------------------------------------------------------------------------------------
# Window and Game set up
window = turtle.Screen()
window.title("Pong by @Mohglii")
...
# Keyboard Binding ( Keyboard Controls )
window.listen()
...
while True:
    ...
    # Border Checking: ( Ball Reflection ( Top & Bottom))
    if ball.ycor() > 290:
        ball.sety(290)
        ball.dy *= -1
    ...
    # Border Checking: ( Ball Reflection (Side to Side) + Score)
    if ball.xcor() > 390:
        ball.goto(0, 0)
        ball.dx *= -1
        score_a += 1
